...
function get(vari, data01) {
 if (vari != undefined) {
  ...
  const url = `http://api.weatherapi.com/v1/forecast.json?key=${API_KEY}&q=${query}&days=7&aqi=yes&alerts=yes
  `;
  fetch(url).then((res) => {
   return res.json();
  }).then((data) => {
  ...
  <div class="text_box">
   ...
   for (var i = 0; i <= 23; i++) {
    ...
    hour_box.innerHTML = `  
          <div class="for_data">
                 ...
                 <span id="temp_min">${today_hour[i].temp_c}°c <br><span id="temp_min" class="mini">${today_hour[i].feelslike_c}°c</span></span>
   ...
   for (let i = 0, j = 0; i <= 6, j <= 7; i++, j++) {
    ...
    <div class="week_for">
       ...
       <img src= ${week_day_data[i].day.condition.icon} width="50rem" height="50rem">
